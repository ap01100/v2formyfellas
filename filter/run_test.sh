...
show_help() {
    ...
    echo "  --sources-file FILE  Указать файл с источниками конфигураций (по умолчанию: $SOURCES_FILE)"
    echo ""
    echo "Примеры:"
    ...
    echo "  $0 --sources-file custom_sources.txt"
    exit 0
}
...
parse_args() {
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            --sources-file)
                if [[ -z "${2:-}" ]]; then
                    echo "ОШИБКА: Отсутствует аргумент для параметра --sources-file" >&2
                    exit 1
                fi
                SOURCES_FILE="$2"
                shift 2
    ...
    # Проверка наличия файла с источниками
    if [[ ! -f "$SOURCES_FILE" ]]; then
        echo "ОШИБКА: Файл с источниками '$SOURCES_FILE' не найден." >&2
        exit 1
    fi
    ...
    echo "  Файл с источниками: $SOURCES_FILE"
}
...
# Функция для проверки, является ли строка base64-кодированной
is_base64_encoded() {
    local data="$1"
    local first_line
    ...
    # Проверяем, соответствует ли строка паттерну base64
    # Base64 содержит только буквы, цифры, +, / и может заканчиваться =
    if [[ "$first_line" =~ ^[A-Za-z0-9+/]+=*$ ]]; then
        # Пытаемся декодировать и проверить, получается ли что-то осмысленное
        # Проверяем наличие протоколов v2ray в декодированных данных
        if echo "$first_line" | base64 -d 2>/dev/null | grep -q -E "^(ss|vmess|trojan|vless)://"; then
            return 0  # Это base64
        fi
    fi
    
    return 1  # Это не base64
}
...
download_and_filter() {
    ...
    # Скачиваем данные во временный файл
    local temp_file="${WORK_DIR}/temp_download.txt"
    if ! curl -fsSL "$url" > "$temp_file"; then
        echo "   ОШИБКА: Не удалось скачать конфиги с ${url}" >&2
        return 1
    fi
    
    # Проверяем первую строку на base64-кодирование
    ...
    if [[ "$is_base64" == "true" ]]; then
        echo "   Обнаружены base64-кодированные данные, декодирую..."
        if cat "$temp_file" | base64 -d | grep --color=never '^[A-Za-z]' >> "$output_file"; then
            echo "   Успешно декодировано и отфильтровано."
        else
            echo "   ОШИБКА: Не удалось декодировать данные из ${url}" >&2
            rm -f "$temp_file"
            return 1
        fi
    else
        # Для обычных источников
        if cat "$temp_file" | grep --color=never '^[A-Za-z]' >> "$output_file"; then
            echo "   Успешно отфильтровано."
        else
            echo "   ОШИБКА: Не удалось отфильтровать данные из ${url}" >&2
            rm -f "$temp_file"
            return 1
        fi
    fi
...
if ask_user "Обновить конфиги из удаленных источников?"; then
    ...
    # Читаем URL из файла с источниками
    source_count=0
    while IFS= read -r url || [[ -n "$url" ]]; do
        ...
        # Генерируем имя источника из URL
        source_name="source_${source_count}"
        if [[ "$url" == *"barry-far"* ]]; then
            source_name="barry-far"
        elif [[ "$url" == *"epodonios"* ]]; then
            source_name="epodonios"
        elif [[ "$url" == *"soroush"* ]]; then
            source_name="soroushmirzaei"
        fi
        
        # Скачиваем и фильтруем конфигурации
        download_and_filter "$url" "$RAW_FILE" "$source_name"
        
        ((source_count++))
    done < "$SOURCES_FILE"
    
    echo "Обработано $source_count источников конфигураций."
else
